...
namespace Google.Maps.Examples.Shared {
  ...
  internal static class ExampleDefaults {
    ...
    public static readonly Bounds DefaultBounds =
        new Bounds(Vector3.zero, new Vector3(800, 0, 800));
    ...
    static ExampleDefaults() {
      ...
      // ground, roads, etc.
      Material wallMaterial = new Material(standardShader) { color = new Color(0f, 0.75f, 1f) };
      Material roofMaterial = new Material(standardShader) { color = new Color(0.29f, 0.62f, 0.53f, 0.5f) };
            roofMaterial.SetFloat("_Mode", 2);

      Material regionMaterial = new Material(baseMapShader) {
        color = new Color(0.29f, 0.52f, 0.53f),
      };
      ...
      segmentMaterial.SetFloat("_Glossiness", 0f);
