namespace PlayMySpace.PMSC.Input
{
    ...
    using PlayMySpace.PMSC.Landmarks;
    ...
    /// </summary>
    public class NetworkedPetController : NetworkBehaviour
    {
        #region Class Member
        ...
        public void PetToMonsterTransformation()
        {
            if (isLocalPlayer)
            {
                PetToMonsterTransformationCmd(gameObject);
            }
        }
        #endregion
        ...
        [Command]
        private void PetToMonsterTransformationCmd(GameObject g)
        {
            //PetToMonsterTransformationClientRpc(g);
            PetMonsterEvent p = Instantiate(petMonsterEventPrefab, new Vector3(268, 0, -101), Quaternion.Euler(0, 35, 0)).GetComponent<PetMonsterEvent>();
            NetworkServer.Spawn(p.gameObject, g);
            p.StartEvent(10);
        }
        ...
        [ClientRpc]
        private void PetToMonsterTransformationClientRpc(GameObject g)
        {
        ...
        [ClientRpc]
        private void ChangeCharacterActiveClientRpc(GameObject character, bool active)
        {
            character.SetActive(active);
        }

        [ClientRpc]
        private void InstantiateObjectClientRpc(GameObject o, Vector3 position, Quaternion rotation, Transform parent)
        {
            Instantiate(o, position, rotation, parent);
        }

        [TargetRpc]
        private void SetMonsterTargetRpc(NetworkConnection conn, GameObject monster)
        {
            GameManager.Instance.PlayerLogicManager.Monster = monster;
            monster.SetActive(false);
        }

        [Command]
        public void ReplacePetWithMonsterCmd()
        {
            GameObject monster = Instantiate(GameManager.Instance.PlayerLogicManager.MonsterPrefab, transform.position, transform.rotation, null);
            NetworkServer.ReplacePlayerForConnection(connectionToClient, monster, true);
            SetMonsterTargetRpc(monster.GetComponent<NetworkIdentity>().connectionToClient, monster);
            monster.SetActive(false);
            ChangeCharacterActiveClientRpc(monster, false);
        }

        [Command]
        public void ChangeCharacterActiveCmd(GameObject character, bool active)
        {
            ChangeCharacterActiveClientRpc(character, active);
        }
        #endregion
    }
}
